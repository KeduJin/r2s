...
if [ -f .env ]; then set -a; source .env; set +a; fi
cd $PROJ_DIR
export USE_SEEDABLE_SAMPLER=false

accelerate launch \
  --config_file accelerate_config/ds_zero2.yaml\
  experiments/train.py \
  --config configs/train/qwen-esm2_35M-qwen3ca_100M-weighting1_1-plddt_filter-use_seedable_false.yaml 2>&1 | tee \
  output/SlurmLogs/qwen-esm2_35M-qwen3ca_100M-weighting1_1-plddt-filter_use_seedable_false.log
