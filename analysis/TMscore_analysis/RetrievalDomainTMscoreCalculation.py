...
def load_all_data_from_tsv(filepath, max_rows=-1):
    ...
    参数:
    - filepath: TSV文件路径，格式为 domains, generated_seq
    - max_rows: 最大读取行数，-1表示读取所有

    返回:
    - data_list: 包含所有行的数据列表，每行为 (domains, generated_seq)
    """
    data_list = []
    ...
    for line_idx, line in enumerate(lines):
        ...
        try:
            ...
            if "', '" in domains_str:
                ...
                # 只有一个domain
                domains = [domains_str.strip("'\"")]

            # 获取生成序列
            generated_seq = parts[1]
        ...
        data_list.append((domains, generated_seq))
...
def main():
    ...
    # 准备任务
    tasks = []
    for idx, (domains, generated_seq) in enumerate(data_list):
    ...
    # 整理结果
    mean_tm_scores = [0.0] * len(data_list)
    all_domain_tm_scores = [[]] * len(data_list)
    ...
    # 计算统计
    overall_mean_tm_score = np.mean([s for s in mean_tm_scores if s > 0])

    print("\n" + "="*60)
    print("计算完成！")
    print("="*60)
    print(f"有效样本数量: {len([s for s in mean_tm_scores if s > 0])}")
