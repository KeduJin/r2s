...
method_suffix=""
if grep -q "generate_simple_with_unique_domain_pieces" "$generation_config" 2>/dev/null; then
    method_suffix="_unique"
fi


if [ -z "$ckpt_path" -o -z "$_config" -o -z "$_test_config" -o -z "$generation_config" ]; then
