...
class DiffusionProteinLanguageModel(nn.Module):
    ...
    def from_pretrained(
        ...
        if not from_huggingface:
            ...
            net = net_class.from_pretrained(
                net_name, local_files_only=True, **net_override
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs = self.net(
            input_ids=input_ids,
            encoder_hidden_states=cross_attention,
    ...
    def compute_loss(self, batch, weighting="constant"):
        raise NotImplementedError(
            "compute_loss is not implemented for DPLM. Should be implemented in the DPLMwDomainConditioning or DPLMwSeqConditioning."
        )
    ...
    def resample(self, _tokens, _scores, ratio, scale):
        ...
        """
        raise NotImplementedError(
            "Resampling is not implemented for DPLM with domain conditioning."
        )
        to_be_resample_idx = []
        resample_input = []
        resample_input_mask = []
    ...
    def forward_decoder(
        ...
        net_out = self.net(
            ...
            encoder_hidden_states=encoder_out,  # fengyuan
            encoder_attention_mask=encoder_mask,  # fengyuan:
        )
    ...
    def generate(
        ...
        partial_masks=None,
        sampling_strategy="gumbel_argmax",  #
        disable_resample=True,  # fengyuan: disable resample
        resample_ratio=0.25,
    ):
        raise NotImplementedError(
            "Generate is not implemented for DPLM with domain conditioning.\n Should be implemented in the DPLMwDomainConditioning or DPLMwSeqConditioning."
        )
        tokenizer = tokenizer
        max_iter = max_iter
