...
from transformers import EsmModel, EsmTokenizer, GenerationConfig

from utils.init_utils import construct_class_by_name
from utils.metrics import Scalar
from utils.perturb_domain_sequence import compute_perturbation_stats
...
class Qwen3CAwDomainConditioning(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        # ### for temp debug
        # self.esm_encoder.encoder.emb_layer_norm_after.weight.requires_grad = False
        # self.esm_encoder.encoder.emb_layer_norm_after.bias.requires_grad = False
        # self.domain_feats_projector.weight.requires_grad = False
        # self.domain_feats_projector.bias.requires_grad = False
        # ###
    ...
    def set_objective_and_metrics(self, stage: str = "train", experiment=None):
        ...
        if stage == "train":
            train_metrics = {
                ...
                # 数据扰动统计
                "domain_boundary_changed_ratio_in_batch": Scalar(dist_sync_on_step=True),
                "domain_boundary_change_ratio": Scalar(dist_sync_on_step=True),
                "domain_mutation_ratio": Scalar(dist_sync_on_step=True),
            }
            val_metrics = {
                "loss": Scalar(dist_sync_on_step=True),
                "domain_loss": Scalar(dist_sync_on_step=True),
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.criterion:
            ...
            loss = out_dict["loss"]
            logging_output = {}
        if "domain_boundary_changed" in kwargs:
            logging_output.update(compute_perturbation_stats(kwargs))
