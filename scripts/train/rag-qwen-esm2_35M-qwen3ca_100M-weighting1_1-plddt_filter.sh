...
if [ -f .env ]; then set -a; source .env; set +a; fi
cd $PROJ_DIRcd $PROJ_DIR
accelerate launch \
  --config_file accelerate_config/ds_zero2.yaml\
  experiments/train.py \
  --config configs/train/rag-qwen-esm2_35M-qwen3ca_100M-weighting1_1-plddt_filter.yaml 2>&1 | tee \
  output/SlurmLogs/rag-qwen-esm2_35M-qwen3ca_100M-weighting1_1-plddt_filter.log
