    ...
    def _build_batches(self):
        ...
        for i, idx in enumerate(self.sampler):
            ...
            if linear <= self.max_tokens and quadratic < self.max_square_tokens:
                ...
                if len(batch) == 0:
                    ...
                    # continue
                    raise ValueError(
                        f"A sample of length {this_length} is too large to fit in a batch "
        ...
        if self.sampler.num_replicas > 1:
            ...
            if len(batches) < num_samples:
                if len(batches) == 0 and num_samples > 0:
                    raise RuntimeError(
                        f"Rank {self.sampler.rank} has 0 batches, while other ranks have up to {num_samples}. "
                        "This is likely due to uneven data distribution. Check your dataset partitioning."
                    )
...
class UniRefHFDataset(Dataset):
    ...
    def __init__(
        ...
        if logger is not None:
            logger.info(f"Dataset size for {split} split: {len(self.indices)}")
        else:
            print(f"Dataset size for {split} split: {len(self.indices)}")
        self.max_len = max_len
...
class DPLMCollater(object):
    ...
    def __call__(self, sequences):
        ...
        batch = {
            ...
            "seq_ids": batch["input_ids"],
            "seq_masks": batch["attention_mask"].bool(),
            "targets": batch["input_ids"].clone(),
        }
        return batch
